...
library Checkpoints {
    ...
    function lowerLookupKey(Trace208 storage self, uint48 key) internal view returns (uint48) {
        uint256 len = self._checkpoints.length;
        uint256 pos = _lowerBinaryLookup(self._checkpoints, key, 0, len);
        return pos == len ? 0 : _unsafeAccess(self._checkpoints, pos)._key;
    }
    ...
    function upperLookupKey(Trace208 storage self, uint48 key) internal view returns (uint48) {
        uint256 len = self._checkpoints.length;
        uint256 pos = _upperBinaryLookup(self._checkpoints, key, 0, len);
        return pos == 0 ? 0 : _unsafeAccess(self._checkpoints, pos - 1)._key;
    }
