...
const stableCoinsAddress = process.env.STABLES_PROXY_ADDRESS; // StableCoins
...
async function grantMinterRole() {
  try {
    ...
    const stableCoins = new ethers.Contract(stableCoinsAddress, contractABI, wallet);

    const minterRole = ethers.keccak256(ethers.toUtf8Bytes('MINTER_ROLE'));

    console.log('Granting minter role...');
    const tx = await stableCoins.grantRole(minterRole, newMinterAddress);
